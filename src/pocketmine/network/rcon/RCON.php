...
class RCON{
	...
	public function __construct(Server $server, $password, $port = 19132, $interface = "0.0.0.0", $threads = 1, $clientsPerThread = 50){
		...
		if($this->socket === false or !socket_bind($this->socket, $interface, (int) $port) or !socket_listen($this->socket)){
			...
			$this->server->getLogger()->info("Trying a new connection");
			//$this->threads = 0;
            $this->stop();
			return new RCON($server, $password, $port, $interface, $threads, $clientsPerThread);
		}
	...
	public function stop(){
		...
		$this->threads = 0;
        socket_write($this->socket, "\x00"); //make select() return
